...
library; the own-radio Zigbee stack is still a skeleton; **working Zigbee is available today
...
| **CC310 (crypto)** | ✅ **Working CC310 hardware acceleration** via the separate **[NiusCrypto](https://github.com/dunknowcoding/ArduinoNRF-Crypto)** library (CRYS runtime on the CryptoCell 310 — SHA-256, AES-CBC/CTR, ECDSA/ECDH P-256, TRNG — plus Oberon for AES-GCM; HW-verified on ProMicro). The in-package [`libraries/CC310/`](hardware/arduinonrf/nrf52/libraries/CC310/) is a compatibility shim that keeps `#include <NrfCC310.h>` working; without NiusCrypto it still compiles but every operation returns `CC310_NOT_VENDORED`. |

...
static int ble_svc_gap_device_name_write_access(struct ble_gatt_access_ctxt *ctxt)
{
    ...
    if (ble_svc_gap_chr_changed_cb_fn != NULL) {
                              ...
                              struct ble_gatt_access_ctxt *ctxt, void *arg)
{
    ...
    case BLE_SVC_GAP_CHR_UUID16_DEVICE_NAME:
        if (ctxt->op == BLE_GATT_ACCESS_OP_READ_CHR) {
            return ble_svc_gap_device_name_read_access(ctxt);
        }
        if (ctxt->op == BLE_GATT_ACCESS_OP_WRITE_CHR) {
            return ble_svc_gap_device_name_write_access(ctxt);
        }
        assert(0);
        return BLE_ATT_ERR_UNLIKELY;
    ...
    case BLE_SVC_GAP_CHR_UUID16_APPEARANCE:
        if (ctxt->op == BLE_GATT_ACCESS_OP_READ_CHR) {
            return ble_svc_gap_appearance_read_access(ctxt);
        }
        if (ctxt->op == BLE_GATT_ACCESS_OP_WRITE_CHR) {
            return ble_svc_gap_appearance_write_access(ctxt);
        }
        assert(0);
        return BLE_ATT_ERR_UNLIKELY;

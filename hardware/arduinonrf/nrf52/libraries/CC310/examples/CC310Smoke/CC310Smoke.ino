...
void setup() {
  ...
  const auto s512 = NrfCC310::sha512(kAbc, sizeof(kAbc), digest512);
  pass &= reportOk("sha512(\"abc\")", s512);
  if (s512 == NrfCC310::CC_OK) {
    const bool sha512Ok = equal(digest512, kSha512Abc, 64);
    Serial.print(F("    match NIST: "));
    Serial.println(sha512Ok ? F("PASS") : F("FAIL"));
    pass &= sha512Ok;
  }
